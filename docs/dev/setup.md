...
- Required: Go `1.22.5` (see `.go-version`)

## Install Go

1. Install Go `1.22.5` from [go.dev](https://go.dev/dl/).
...
Expected output should contain `go1.22.5`.
...
- `toolchain not available`:
  Install Go `1.22.5` locally and rerun commands; avoid relying on auto-download.
- `go.mod/go.sum changed in CI`:
  Run `go mod tidy` locally and commit the resulting `go.mod` and `go.sum`.
- `tests fail only in CI`:
  Check `go version` locally and ensure it matches CI (`1.22.5`).
- Offline development:
