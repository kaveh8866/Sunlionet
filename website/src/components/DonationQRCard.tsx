...
export function DonationQRCard({ qrValue, addresses }: DonationQRCardProps) {
  ...
  return (
    ...
    >
      <h2 id="donation-title" className="text-2xl font-extrabold tracking-tight text-foreground">
        Direct Anonymous Donation
      </h2>
      <p className="mt-3 text-muted leading-relaxed">
        Scan to donate BTC / XMR / USDT or copy the address. One-time or recurring, every contribution helps keep
      ...
      <p className="mt-4 text-center text-xs text-muted-foreground">
        QR points to the first address in the list (recommended: XMR).
      </p>
            ...
            {addresses.map((item) => (
              <tr key={`${item.symbol}-${item.network}`}>
                ...
                <td className="px-4 py-3">
                  ...
                  >
                    {copied === item.symbol ? "Copied" : "Copy"}
                  </button>
                </td>
              </tr>
            ))}
