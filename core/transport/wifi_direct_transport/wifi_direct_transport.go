...
func NewSim(net *bridge_node_logic.SimNet, node identity_manager.NodeID) *Transport {
	ep := net.Register(bridge_node_logic.MediumWiFi, node, 0)
	ep.SetAvailable(true)
	return ep
}

func LinkScore(_ identity_manager.NodeID, _ time.Time) float64 { return 0.9 }
