...
var (
	ErrCorruptStore     = errors.New("community: corrupt store")
	ErrDecryptionFailed = errors.New("community: decryption failed")
)
...
func NewStore(dbPath string, masterKey []byte) (*Store, error) {
	if len(masterKey) != 32 {
		return nil, errors.New("community: masterKey must be 32 bytes for AES-256")
	}
...
func (s *Store) Save(st *State) error {
	if s == nil {
		return errors.New("community: store is nil")
	}
	if st == nil {
