...
elif [ -f "${tmpdir}/shadownet-inside" ]; then
  sudo install -m 0755 "${tmpdir}/shadownet-inside" /usr/local/bin/sunlionet-inside
fi

if [ -f "${tmpdir}/sunlionet-outside" ]; then
  sudo install -m 0755 "${tmpdir}/sunlionet-outside" /usr/local/bin/sunlionet-outside
elif [ -f "${tmpdir}/shadownet-outside" ]; then
  sudo install -m 0755 "${tmpdir}/shadownet-outside" /usr/local/bin/sunlionet-outside
fi
